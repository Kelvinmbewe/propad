...
4. Apply database migrations: `docker compose exec api pnpm prisma migrate deploy`.
5. Seed baseline demo users: `docker compose exec api pnpm prisma db seed`.
...
Seed baseline demo users for login (runs inside Docker):
