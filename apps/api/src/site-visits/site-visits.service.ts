...
export class SiteVisitsService {
  ...
  private readonly GPS_THRESHOLD_KM = 20.0; // 20km threshold for approval
  ...
  async requestVisit(userId: string, propertyId: string) {
    ...
    try {
      ...
      this.logger.error(
        `Failed to auto-assign officer for site visit ${visit.id}:`,
        error,
      );
  ...
  async autoAssignNearbyOfficer(siteVisitId: string): Promise<void> {
    ...
    if (visit.assignedModeratorId) {
      this.logger.log(
        `Site visit ${siteVisitId} already assigned to ${visit.assignedModeratorId}`,
      );
      return;
    }
    ...
    if (!propertyLat || !propertyLng) {
      this.logger.warn(
        `Property ${visit.propertyId} has no GPS coordinates, falling back to admin assignment`,
      );
    ...
    if (nearestOfficer) {
      ...
      this.logger.log(
        `Auto-assigned site visit ${siteVisitId} to officer ${nearestOfficer.id} (${nearestOfficer.distance.toFixed(2)}km away)`,
      );
    } else {
      // Fallback to admin
      await this.assignToAdmin(siteVisitId);
  ...
  private async assignToAdmin(siteVisitId: string): Promise<void> {
    ...
    if (admin) {
      ...
      this.logger.log(
        `Assigned site visit ${siteVisitId} to admin ${admin.id} (fallback)`,
      );
    } else {
  ...
  async assignModerator(
    ...
    if (
      ...
      visit.status === SiteVisitStatus.FAILED
    ) {
      throw new BadRequestException(
        "Cannot reassign a completed or failed visit",
      );
    }
    ...
    if (
      ...
      moderator.role !== Role.VERIFIER
    ) {
      throw new BadRequestException(
        "Officer must have role ADMIN, MODERATOR, or VERIFIER",
      );
    }
  ...
  async completeVisit(
    ...
    if (visit.assignedModeratorId !== moderatorId) {
      throw new ForbiddenException(
        "Only assigned officer can complete this visit",
      );
    }
    ...
    const targetLat = visit.verificationItem?.gpsLat ?? null;
    const targetLng = visit.verificationItem?.gpsLng ?? null;

    if (!targetLat || !targetLng) {
      throw new BadRequestException(
        "Location confirmation GPS is required before completing a site visit",
      );
    }
    ...
    const visitStatus = isCloseEnough
  ...
  async declineVisit(visitId: string, moderatorId: string, reason?: string) {
    ...
    if (visit.assignedModeratorId !== moderatorId) {
      throw new ForbiddenException(
        "Only assigned officer can decline this visit",
      );
    }
    ...
    const declineNote = reason?.trim()
      ? `Visit declined: ${reason.trim()}`
      : "Visit declined by assigned officer";
  ...
  private async recalculateVerificationRequestStatus(
    ...
    const statusCounts = {
      APPROVED: allItems.filter(
        (i: any) => i.status === VerificationItemStatus.APPROVED,
      ).length,
      REJECTED: allItems.filter(
        (i: any) => i.status === VerificationItemStatus.REJECTED,
      ).length,
      SUBMITTED: allItems.filter(
        (i: any) => i.status === VerificationItemStatus.SUBMITTED,
      ).length,
      PENDING: allItems.filter(
        (i: any) => i.status === VerificationItemStatus.PENDING,
      ).length,
    };
