...
export class RankingService {
  ...
  ): { property: Property; score: number; breakdown: any }[] {
  ...
  async rankListingsAsync(
    properties: Property[],
    params: RankingParams,
  ): Promise<{ property: Property; score: number; breakdown: any }[]> {
    const ranked = await Promise.all(
      properties.map(async (property) => {
        ...
        const riskMultiplier =
          this.riskService.getRiskPenaltyMultiplier(riskScore);

        // --- ETHICAL MONETIZATION (Phase 10) ---
        // 1. Fetch active boosts
        const activeBoosts = await this.monetizationService.getActiveBoosts(
          "PROPERTY",
          property.id,
          riskScore,
        );
        ...
        let boostEffect = 0;
        if (
          activeBoosts.some((b: { type: string }) => b.type === "LISTING_BOOST")
        ) {
          boostEffect += 15;
        } else if (
          activeBoosts.some(
            (b: { type: string }) => b.type === "FEATURED_LISTING",
          )
        ) {
          boostEffect += 10;
        }
  ...
  calculateScoreBreakdown(property: Property, params: RankingParams) {
    ...
    const listingStatus = property.status;
    ...
    if (listingStatus === "PENDING_VERIFY" || listingStatus === "PUBLISHED") {
      statusMultiplier = 0.7;
    }

    total = (total + verificationBonus) * statusMultiplier;
    ...
    return {
      total: Math.min(100, Math.max(0, Math.ceil(total))),
      components: {
        trust: trustScore,
  ...
  async rankWithPromotedListings(
    ...
    promotedListings: Array<
      Property & { isPromoted: boolean; campaignId: string }
    >,
  ): Promise<
    {
      property: Property;
  ...
  > {
    ...
    const rankedOrganic = await this.rankListingsAsync(
      organicProperties,
      params,
    );
