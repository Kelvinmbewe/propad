...
export class DashboardService {
  ...
  async getOverview(userId: string, role: string) {
    switch (role) {
      ...
      case Role.ADMIN:
        ...
        return {
          type: "ADMIN",
          message: "Use /admin/metrics/overview for full stats",
        };
      default:
        return this.getUserOverview(userId);
    }
  ...
  private async getAgentOverview(userId: string) {
    ...
    const [activeListings, totalViews, newLeads, pendingCommission] =
      await Promise.all([
  ...
  private async getLandlordOverview(userId: string) {
    const [ownedProperties, activeTenants, totalApplications] =
      await Promise.all([
        this.prisma.property.count({
    ...
    return {
      ...
      activeTenants,
      occupancyRate:
        ownedProperties > 0 ? (activeTenants / ownedProperties) * 100 : 0,
      totalApplications,
    };
  }
