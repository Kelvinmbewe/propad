...
export class VerificationsService {
  ...
  async getVerificationQueue() {
    try {
      ...
      const userIds = requests
        .filter(
          (r: VerificationRow) => r.targetType === "USER" && r.targetUserId,
        )
        .map((r: VerificationRow) => r.targetUserId!);

      const agencyIds = requests
        .filter(
          (r: VerificationRow) => r.targetType === "COMPANY" && r.agencyId,
        )
      ...
      users.forEach((u: UserLite) => {
        userMap[u.id] = u;
      });

      const agencyMap: Record<string, AgencyLite> = {};
      agencies.forEach((a: AgencyLite) => {
        agencyMap[a.id] = a;
      });
      ...
      const queue = requests.map((req: VerificationRow) => {
        ...
        const isPaid = !!req.property?.listingPayments?.some(
          (p) => p.status === "PAID",
        );
        ...
        return {
          ...
          requesterName:
            req.requester?.name || req.requester?.email || "System",
        };
      });
      ...
      return queue.sort(
        (a: VerificationQueueItem, b: VerificationQueueItem) => {
  ...
  async findAllRequests(filters: {
    ...
    return this.prisma.verificationRequest.findMany({
      where: {
        ...
        ...(filters.status
          ? { status: filters.status as VerificationStatus }
          : {}),
      },
      include: {
  ...
  async reviewItem(
    requestId: string,
    itemId: string,
    dto: ReviewVerificationItemDto,
    actor: AuthContext,
  ) {
    try {
      const action =
        dto.status === VerificationItemStatus.APPROVED ? "APPROVE" : "REJECT";
      ...
      if (item.verificationRequestId !== requestId) {
        throw new BadRequestException(
          "Item does not belong to the specified verification request",
        );
      }
      ...
      if (dto.status === VerificationItemStatus.REJECTED && !dto.notes) {
        throw new BadRequestException(
          "Rejection reason (notes) is required when rejecting an item.",
        );
      }

      if (item.status === dto.status) {
        this.logger.log(
          `Item ${itemId} already has status ${dto.status}, skipping update`,
        );
        return this.prisma.verificationRequestItem.findUnique({
          where: { id: itemId },
        });
      }

      const { updatedItem, propertyId } = await this.prisma.$transaction(
        async (tx) => {
          ...
          const statusCounts = {
            ...
            SUBMITTED: allItems.filter(
              (i: VerificationRequestItem) =>
                i.status === VerificationItemStatus.SUBMITTED,
            ).length,
            PENDING: allItems.filter(
              (i: VerificationRequestItem) =>
                i.status === VerificationItemStatus.PENDING,
            ).length,
          };
          ...
          if (statusCounts.REJECTED > 0) {
            ...
            statusCounts.APPROVED === allItems.length &&
            allItems.length > 0
          ) {
          ...
          const updatedItem = await tx.verificationRequestItem.findUnique({
            where: { id: itemId },
          });

          let approvedPropertyId: string | null = null;

          const finalAllItems = await tx.verificationRequestItem.findMany({
            where: { verificationRequestId: requestId },
          });
          const allApproved = finalAllItems.every(
          ...
          if (allApproved) {
            ...
            if (req && req.status !== "APPROVED") {
              ...
              if (req.targetType === "PROPERTY" && req.propertyId) {
                ...
                approvedPropertyId = req.propertyId;
              } else if (req.targetType === "USER" && req.targetUserId) {
          ...
          return { updatedItem, propertyId: approvedPropertyId };
        },
      );
      ...
      this.logger.error("[VERIFY ITEM ERROR]", error as Error);

      throw new BadRequestException(
        error instanceof Error
          ? error.message
          : "Failed to review verification item",
      );
    }
  }
  ...
  async recalculatePropertyScore(propertyId: string) {
    ...
    for (const item of approvedItems) {
      if (item.type === "PROOF_OF_OWNERSHIP") totalScore += 50;
        ...
        if (
          item.notes?.includes("On-site visit requested") ||
          item.notes?.includes("On-site visit confirmed")
        ) {
          totalScore += 50;
        } else {
          totalScore += 30;
    ...
    (requests as (VerificationRequest & { items: any[] })[]).forEach((r) => {
      r.items.forEach((i: any) => {
        if (i.status === "APPROVED") {
          if (i.type === "IDENTITY_DOC") {
            score += 40;
            hasId = true;
          }
          if (i.type === "SELFIE_VERIFICATION") {
            score += 30;
            hasSelfie = true;
          }
          if (i.type === "PROOF_OF_ADDRESS") {
            score += 30;
            hasAddress = true;
          }
        }
  ...
  async assignItem(
    ...
    const item = request.items.find(
      (i: VerificationRequestItem) => i.id === itemId,
    );
  ...
  async createRequest(
    ...
    data: {
      targetType: VerificationType;
      targetId: string;
      items: { type: any; evidenceUrls?: string[]; notes?: string }[];
    },
  ) {
    ...
    if (data.targetType === "PROPERTY") createData.propertyId = data.targetId;
      ...
      createData.targetUserId = data.targetId;
