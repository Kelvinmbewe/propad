...
export class IncentivesDashboardService {
    ...
    async getTopEarners(
        ...
        const earnerMap = new Map<string, { rewards: number; commissions: number; rewardCount: number; commissionCount: number }>();
        ...
        for (const c of commissionEarnings) {
            const existing = earnerMap.get(c.agentId) || { rewards: 0, commissions: 0, rewardCount: 0, commissionCount: 0 };
    ...
    async getSourceDistribution(): Promise<SourceDistribution[]> {
        ...
        const total = distributions.reduce(
            (sum: number, d: { _sum: { amountCents: number | null } }) => sum + (d._sum.amountCents || 0),
            0
        );

        return distributions.map((d: { sourceType: string; _sum: { amountCents: number | null }; _count: number }) => ({
            source: d.sourceType,
