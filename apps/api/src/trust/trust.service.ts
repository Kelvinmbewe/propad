...
export class TrustService {
  ...
  async calculatePropertyTrust(propertyId: string): Promise<number> {
    ...
    const verifScore = Math.min(
      40,
      Math.ceil((property.verificationScore || 0) / 2),
    );
    ...
    if (property.propertyRatings.length > 0) {
      const avg =
        property.propertyRatings.reduce(
          (a: number, b: any) => a + b.rating,
          0,
        ) / property.propertyRatings.length;
    ...
    const activityScore = Math.min(
      20,
      Math.floor(property.activityLogs.length / 5),
    );
    ...
    const total = Math.min(
      100,
      Math.ceil(verifScore + ratingScore + activityScore + paymentScore),
    );
  ...
  async calculateUserTrust(userId: string): Promise<number> {
    ...
    try {
      ...
      // Ignore persistence failures for score read
    }
  ...
  async calculateCompanyTrust(agencyId: string): Promise<number> {
    ...
    if (agency.reviews.length > 0) {
      const avg =
        agency.reviews.reduce((a: number, b: any) => a + b.rating, 0) /
