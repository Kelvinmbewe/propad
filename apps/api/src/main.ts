...
async function bootstrap() {
  ...
  const {
    ...
    CurrencyEnum,
  } = await import("@propad/config");
  if (!PropertyTypeEnum) throw new Error("PropertyTypeEnum missing at runtime");
  if (!PropertyFurnishingEnum)
    throw new Error("PropertyFurnishingEnum missing at runtime");
  ...
  if (!PropertyAvailabilityEnum)
    throw new Error("PropertyAvailabilityEnum missing at runtime");
