...
const createCssVar = (token: string, prefix: string) =>
  `var(--${prefix}${token})`;

export const auroraTailwindPreset = ({
  cssVariablePrefix = "aurora-",
}: AuroraTailwindOptions = {}): TailwindConfig => {
  ...
  return {
    ...
    theme: {
      extend: {
        ...
        animation: {
          "aurora-fade-up":
            "aurora-fade-up 450ms var(--aurora-motion-ease, cubic-bezier(0.34, 1.56, 0.64, 1)) both",
