...
const baseServerSchema = z.object({
  ...
  ADSERVER_ENABLED: z.coerce
    .boolean()
    .default(defaultServerEnv.ADSERVER_ENABLED),
  ADSERVER_REWARD_SHARE: z.coerce
    .number()
    .min(0)
    .max(1)
    .default(defaultServerEnv.ADSERVER_REWARD_SHARE),
  VAT_PERCENT: z.coerce
    .number()
    .min(0)
    .max(100)
    .default(defaultServerEnv.VAT_PERCENT),
  ...
  WALLET_MIN_PAYOUT_CENTS: z.coerce
    .number()
    .min(0)
    .default(defaultServerEnv.WALLET_MIN_PAYOUT_CENTS),
  WALLET_EARNINGS_COOL_OFF_DAYS: z.coerce
    .number()
    .min(0)
    .max(30)
    .default(defaultServerEnv.WALLET_EARNINGS_COOL_OFF_DAYS),
  WALLET_MAX_PAYOUTS_PER_DAY: z.coerce
    .number()
    .min(1)
    .max(10)
    .default(defaultServerEnv.WALLET_MAX_PAYOUTS_PER_DAY),
  AGGREGATION_CRON_NIGHTLY: z
    .string()
    .default(defaultServerEnv.AGGREGATION_CRON_NIGHTLY),
  AGGREGATION_CRON_INCREMENTAL: z
    .string()
    .default(defaultServerEnv.AGGREGATION_CRON_INCREMENTAL),
  CACHE_TTL_METRICS_SECONDS: z.coerce
    .number()
    .min(10)
    .max(3600)
    .default(defaultServerEnv.CACHE_TTL_METRICS_SECONDS),
  ...
  NO_STATIC_METRICS: z.coerce
    .boolean()
    .default(defaultServerEnv.NO_STATIC_METRICS),
  NO_DUMMY_LINKS: z.coerce.boolean().default(defaultServerEnv.NO_DUMMY_LINKS),
  CONFIG_HOT_RELOAD: z.coerce
    .boolean()
    .default(defaultServerEnv.CONFIG_HOT_RELOAD),
  NODE_ENV: z
    .enum(["development", "production", "test"])
    .default("development"),
});

const serverSchema = baseServerSchema.refine(
  (data) => {
  ...
  {
    message:
      "Production environment detected but using development secrets! Please update JWT_SECRET, NEXTAUTH_SECRET, and other keys.",
...
export function getServerApiBaseUrl(): string {
  ...
  throw new Error(
    "API base URL is not configured. Please set INTERNAL_API_BASE_URL or NEXT_PUBLIC_API_BASE_URL",
  );
}
